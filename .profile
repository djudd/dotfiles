...
export AWS_ACCESS_KEY_ID=$ACADEMIA_ACCESS_KEY
export AWS_SECRET_ACCESS_KEY=$ACADEMIA_SECRET_KEY

export ACADEMIA_APP=~/src/academia-edu/academia-app
export ACADEMIA_APP_VAGRANT=~/src/academia-app-vagrant
...
export ACADEMIA_DATALYTICS=~/src/academia-edu/academia-datalytics
...
if [ -n "$BASH" ] && [ -f ~/.git-completion.bash ]; then
  . ~/.git-completion.bash
fi
